...
@section('title', 'APAL AI – ' . ($currentBatch->subject ?? 'Dashboard'))
...
<div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:2.5rem;flex-wrap:wrap;gap:1rem;">
    <div>
        <h1 style="font-size:2rem;font-weight:800;letter-spacing:-.02em;">
            {{ $currentBatch ? $currentBatch->subject : 'Dashboard' }}
        </h1>
        <p style="color:var(--text-muted);margin-top:.25rem;">
            {{ $currentBatch ? $currentBatch->teacher_name . ' · ' . $currentBatch->class_name : 'Buat sesi evaluasi baru di bawah.' }}
        </p>
    </div>
...
{{-- ══════════════════════════════════════════════════════════
     CREATE BATCH FORM (shown when no batch / collapsed after)
══════════════════════════════════════════════════════════ --}}
<div class="glass-card" style="margin-bottom:3rem;">
    <h2 style="font-size:1.25rem;font-weight:700;margin-bottom:.4rem;">Buat Sesi Evaluasi Baru</h2>
    <p style="color:var(--text-muted);font-size:.9rem;margin-bottom:2rem;">Isi identitas sesi terlebih dahulu sebelum menambahkan kelompok soal.</p>
    ...
    <form action="{{ route('batches.store') }}" method="POST">
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1.25rem;">
            <div>
                ...
                <input name="school_name" class="form-input" placeholder="SMA Negeri 1 Malang" required>
            </div>
            <div>
                <label class="form-label">Kelas / Rombel</label>
                <input name="class_name" class="form-input" placeholder="XI IPA 3" required>
            </div>
            <div>
                <label class="form-label">Nama Guru</label>
                <input name="teacher_name" class="form-input" placeholder="Budi Santoso, S.Pd" required>
            </div>
            <div>
                <label class="form-label">Mata Pelajaran</label>
                <input name="subject" class="form-input" placeholder="Matematika" required>
            </div>
            <div>
                <label class="form-label">Topik Materi</label>
                <input name="topic" class="form-input" placeholder="Integral Tentu" required>
            </div>
            <div>
            ...
            <div style="grid-column:1/-1;">
                <label class="form-label">Batasan Materi (Ruang Lingkup)</label>
                <textarea name="material_scope" class="form-input" rows="2" placeholder="Contoh: Bab 5 – Integral hal. 120-145. Tidak mencakup integral parsial."></textarea>
            </div>
            <div style="grid-column:1/-1;display:flex;justify-content:flex-end;padding-top:.5rem;border-top:1px solid var(--border);">
...
{{-- ══════════════════════════════════════════════════════════
     ADD QUESTION GROUP (only when batch exists)
══════════════════════════════════════════════════════════ --}}
@if($currentBatch)
<div class="add-group-card">
    ...
    <p style="color:var(--text-muted);font-size:.85rem;margin-bottom:1.75rem;">
        Tambahkan satu kelompok soal ke sesi <strong>{{ $currentBatch->subject }}</strong>.
        Kamu bisa tambah beberapa kelompok (Pilgan 20, Esai 5, dll).
    </p>

    <form action="{{ route('groups.store', $currentBatch->id) }}" method="POST">
        @csrf
...
══════════════════════════════════════════════════════════ --}}
@if($currentBatch->questionGroups->count() > 0)
    @foreach($currentBatch->questionGroups as $group)
...
@endif
@else
    <div style="text-align:center;padding:8rem 0;color:var(--text-muted);opacity:.4;">
        ...
        <h3 style="margin-top:1.5rem;">Belum ada sesi. Buat sesi di atas.</h3>
    </div>
@endif
@endsection
...
<div id="printModal" class="print-modal">
    ...
    <div class="paper-a4">
        ...
        <div class="paper-header-box">
            ...
            <p style="font-size:1.1rem;font-weight:bold;">{{ $currentBatch->school_name }}</p>
            <p style="font-size:.95rem;">Tahun Pelajaran 2024/2025</p>
        </div>

        <div class="paper-meta-row">
            <div><strong>Mata Pelajaran :</strong> {{ $currentBatch->subject }}</div>
            <div><strong>Topik :</strong> {{ $currentBatch->topic }}</div>
            <div><strong>Kelas :</strong> {{ $currentBatch->class_name }}</div>
        </div>
        <div class="paper-meta-row">
            <div><strong>Guru :</strong> {{ $currentBatch->teacher_name }}</div>
            <div><strong>Jenjang :</strong> {{ $currentBatch->school_level }}</div>
        ...
        @if($currentBatch->material_scope)
            <div style="border:1px solid #000;padding:1rem;margin-bottom:2rem;font-size:.9rem;">
                <strong>Batasan Materi:</strong> {{ $currentBatch->material_scope }}
            </div>
        @endif
