<aside class="sidebar">
        ...
        @forelse($batches as $batch)
            <li class="nav-item">
                <a href="{{ route('batches.show', $batch->id) }}"
                   class="nav-link {{ (isset($currentBatch) && $currentBatch->id === $batch->id) ? 'active' : '' }}">
    ...
    <div style="margin-top: 2rem; border-top: 1px solid var(--border); padding-top: 2rem;">
        <a href="{{ route('stats.index') }}"
           class="nav-link {{ request()->routeIs('stats.index') ? 'active' : '' }}"
           style="{{ request()->routeIs('stats.index') ? '' : 'opacity: 0.6;' }}">
